...
function CreateTrip() {
    ...
    const OnGenerateTrip=async()=>{
        ...
        if(formData?.days>5&&!formData?.location||!formData?.hotelName||!formData?.budget||!formData?.traveler)
        {
        ...
        .replace('{hotelName]', formData?.hotelName.label)
        .replace('{totalDays}', formData?.days)
  ...
  return (
    <div className='sm:px-10 md:px-32 lg:px-56 xl:px-80 px-5 mt-10'>
        ...
        <div className='flex flex-col gap-9'>
            <div>
                <h2 className='text-xl my-3 font-medium'>Hotel address?</h2>
                <GooglePlacesAutocomplete
                    apiKey={import.meta.env.VITE_GOOGLE_PLACE_API_KEY}
                    selectProps={{
                        place, 
                        onChange:(v)=>{setPlace(v);handleInputChange('hotelName', v);},
                    }}
                />
            </div>
        </div>
